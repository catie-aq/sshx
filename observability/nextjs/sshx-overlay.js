...
(function () {
  ...
  function cleanFilePath(raw) {
    ...
    // Strip leading ./
    p = p.replace(/^\.\//, '');
    // Strip workspaceRoot prefix if available (absolute path → relative)
    var wsRoot = window.__sshxWorkspaceRoot;
    if (wsRoot && p.startsWith(wsRoot)) {
      p = p.slice(wsRoot.length);
    }
    // Strip leading /
    p = p.replace(/^\//, '');
    return p || null;
  ...
  function getComponentInfo(node) {
    ...
    // Normalize the file path: strip absolute prefix down to project-relative.
    if (file) {
      file = cleanFilePath(file) || file;
    }
